...
password = "Test@123"
print(email_address)
sign_up_url = 'https://together4healthva-vrp-qa-rel.qak8s.vibrenthealth.com/'
login_url = 'https://together4healthva-vrp-qa-rel.qak8s.vibrenthealth.com/login'
...
def test_allow_ehr_access_adult_pediatric_ent_kalamazoo_epic_r4():
    chrome_driver = webdriver.Chrome()
    chrome_driver.get(login_url)
    chrome_driver.maximize_window()
    chrome_driver.implicitly_wait(30)
    ...
    for i in range(2):
        chrome_driver.find_element(By.XPATH, "//button[@data-target='@form|button|continue']").click()
    chrome_driver.find_element(By.XPATH, "//div[@aria-label = 'Adult & Pediatric Ear, Nose & Throat - Kalamazoo']").click()
    for i in range(3):
        chrome_driver.find_element(By.XPATH, "//button[@data-target='@form|button|continue']").click()
    time.sleep(20)
    chrome_driver.switch_to.window(chrome_driver.window_handles[1])
